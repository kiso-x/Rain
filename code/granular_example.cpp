...
*
* \author Henrik von Coler & Marquis
*
...
GranularExample::GranularExample(std::string filename, int win_size, int nWindows)
{
    ...
    // grainer array = new GrainPlayer(filename, fs, oscman, (win_size (hardcoded)), (nWindows (hardcoded)))
...
int GranularExample::process (jack_nframes_t nframes)
{
    ...
    grainer->set_relative_distance(oscman->get_pos());
    std::cout << oscman->get_pos() << std::endl;
    // loop over all samples of output buffer
    for(int sampCNT=0; sampCNT<nframes; sampCNT++)
    {
        /* double current_sum = 0;
        // Hier wird dann ein Array von grainern abgefragt.
        for(int i = 0; i < grainer::size, i++ )
        {
           current_sum += grainer[i]->get_sample();
        }

        // Hier müsste dann noch Filter & VCA kommen
        out[0][sampCNT] = current_sum/4; */
